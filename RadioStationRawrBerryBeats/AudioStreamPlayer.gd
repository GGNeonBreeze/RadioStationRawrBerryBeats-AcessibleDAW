...
# play / rec
	...
	# same with the ones for loading and saving projs (save being
	# top left, load being top right)
	$ProjectSave.position.y = -400
	$ProjectSave.position.x = -700

	$TrackColor.position.y = -400
	$TrackColor.position.x = 600	
	
	$TrackInstrument.position.y = 350
	$TrackInstrument.position.x = -700
	# ^ note to self for these see if you can get the camera sizes
	# (and with adjustable cams these prob need to be set later
	
	
	$MoveTo.position.x = 600
	$MoveTo.position.y = 350
...
func _draw():
	...
	if shiftingTracks == true:
		draw_string(default_font, Vector2(0, position.y - 40), str(currentTrack),0, 90, 22)
