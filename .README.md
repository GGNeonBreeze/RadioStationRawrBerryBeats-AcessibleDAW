...
- To download: you can download/clone the repository. If you are unfamiliar with using git hub, click the green "code" button near the middle of the window, and then press "download zip" at the bottom of the drop down, navigate to your downloads folder on your computer (vaires on the computer but its generally in an app called "files" or "finder") and right click the zipped folder, then unzip it. You also need Godot (at least for these early test versions of the program) which can be downloaded similarly from here: https://godotengine.org/ * there is a known bug with running this in the latest version of Godot so you might need to use 4.4 if you want the best results
- To import into Godot: With both downloaded, launch the Godot app and click the "Import" button in the top left, navigate to the RadioStationRawrBerryBeats folder and select the "project.godot" file. This will take you to the editor, but if you want to run the program, click the play button in the top right. Let me know if there are any problems with the set up/initialization (feel free to describe them as issues here on git for now). This version is also mobile compatible, but not as explicitly designed around it right now, which is to say in the future we will add more to that build.
